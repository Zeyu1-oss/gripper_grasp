...
DEFAULT_TARGET = "lego_01_geom"

XY_ALIGN_TOL      = 0.005      # XY 对齐阈值 (m)
DOWN_SAFE_GAP     = 0.0015     # 指尖底部相对乐高“底面”的安全缝隙 (m)
LIFT_STEP_MAX     = 0.004      # 每步最大 Z 关节改变量 (m)
CLOSE_STEP        = 0.005      # 每步夹爪改变量
GRASP_FORCE_TH    = 0.1        # 左右指与目标的法向力阈值 (N)
...
def main():
    ...
    if None in (aid_x, aid_y, aid_lift, aid_left, aid_right, aid_rot,
                gid_left, gid_right, gid_palm, gid_target):
        print("[ERROR] 名称映射失败")
        return

    # 先张开
    lo_l, hi_l = model.actuator_ctrlrange[aid_left]
    lo_r, hi_r = model.actuator_ctrlrange[aid_right]
    data.ctrl[aid_left]  = hi_l  
    data.ctrl[aid_right] = hi_r      # 右手指张开
        ...
        while viewer.is_running():
            ...
            if now - last_print >= PRINT_PERIOD:
                print(f"[{phase:10s}] palm=({palm[0]:+.3f},{palm[1]:+.3f},{palm[2]:.3f}) "
                      ...
                      f"ctrl(left)={data.ctrl[aid_left]:+.3f} ctrl(right)={data.ctrl[aid_right]:+.3f} "
            ...
            if phase == "align_xy":
                ...
                print(f"abs(palm[0] - target[0])={abs(palm[0]-target[0]):.4f}, "
                     f"abs(palm[1] - target[1])={abs(palm[1]-target[1]):.4f}")
                if abs(palm[0] -  data.ctrl[aid_x]) < XY_ALIGN_TOL or \
                ...
                if abs(err) < 0.05:
                    phase = "descend"
#乐高高度是对的，是一个很小的数字，finger_bot=palm初始高度-（finger中间-一半）
            elif phase == "descend":
                err = finger_bot_z-z_goal
                step = np.clip(err, -LIFT_STEP_MAX, LIFT_STEP_MAX)
                data.ctrl[aid_lift] = clamp_to_ctrlrange(model, aid_lift, data.ctrl[aid_lift] - step)- 0.323
                if abs(err) < 0.005:
                    phase = "close"

            elif phase == "close":
                data.ctrl[aid_left]  = clamp_to_ctrlrange(model, aid_left,
                                          data.ctrl[aid_left] - CLOSE_STEP)
                data.ctrl[aid_right] = clamp_to_ctrlrange(model, aid_right,
                                          data.ctrl[aid_right] - CLOSE_STEP)

                leftF, rightF, hitL, hitR = sum_finger_forces_with_target(
                    model, data, gid_left, gid_right, gid_target
                )

                if (hitL and hitR and data.ctrl[aid_right] == -1) or (data.ctrl[aid_right] == -1):
                    phase = "lift"
