...
OPEN_CMD       = 0.8
CLOSE_CMD      = -0.6
SETTLE_SECONDS    = 3.0   # 纯时间兜底：至少等待这么久
SETTLE_VEL_THRESH = 0.02  # 动态判稳阈值：系统总速度范数
SETTLE_STABLE_STEPS = 200 # 连续多少步低速视为“稳定
CONTACT_TH      = 0.4
BOTH_CONTACT_TH = 0.5
...
def compute_grasp_yaw(model, data, gid_tgt, state, longest_axis):
    R = data.geom_xmat[gid_tgt].reshape(3, 3)
    x, y, z = R[:, 0], R[:, 1], R[:, 2]   # lego 的局部 x/y/z 轴在世界系中的方向

    if state == "vertical":
        # 竖直：直接让 palm-x ∥ lego-x（同时也能保证 palm-y ∥ lego-y）
        yaw = np.arctan2(x[1], x[0])

        # 同向性检查：确保 palm-y 和 lego-y 不反向（避免 180° 翻转歧义）
        ...
        v = z[:2] #v是 lego-z 在 XY 平面投影
        n = np.linalg.norm(v)
        if n < 1e-9:
            return 0.0
        v /= n

        # 关键：palm-y = [-sinψ, cosψ]，要与 v 同向 ⇒ ψ = atan2(vy, vx) - π/2
