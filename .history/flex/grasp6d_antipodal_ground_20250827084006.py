...
# -------- 参数（可按需改）--------
SAMPLES     = 25000      # 表面采样数
MU          = 0.6        # 摩擦系数 -> 圆锥半角 atan(mu)
ANGLE_TOL   = np.deg2rad(8.0)
CLEARANCE   = 0.0015     # 指尖走廊的最小净空
FINGER_LEN  = 0.03       # 指深（接近方向预留）
TOPK        = 200
VOXEL_MM    = 3.0        # 结果去重体素（mm）
...
def score_grasp(n, nj, axis, cone_half):
    ...
    return 0.6*fc_margin + 0.4*normal_sym
...
def synthesize(mesh, table_z=0.0, mu=MU, n_samples=SAMPLES,
    ...
    for i in range(len(P)):
        ...
        for w, j in cand[:15]:  # 每个点最多试 15 个最近对点
            pj, nj = P[j], unit(N[j])
            axis = unit(pj - p)
            ...
            # y_hat 沿两接触点连线（闭合方向）
            y_hat = axis
            # 接近方向：优先 -(n + nj)；几何退化时，取与 y_hat 正交且“朝上”的方向
            sumn = n + nj
            ...
            # 打分
            s = score_grasp(n, nj, axis, cone_half)
            # 离地越高越好（轻微加权）
            s += 0.15*np.clip((center[2]-table_z)/max(ext[2],1e-3), 0, 1)
