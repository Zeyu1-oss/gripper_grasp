...
def compute_grasp_yaw(model, data, gid_target):
    """yaw角度"""
    R = data.geom_xmat[gid_target].reshape(3, 3) #rotation matrix of world frame
    size = model.geom_size[gid_target]   # 半边长 (x,y,z)

    axis = np.argmax(size)  # 0=x, 1=y, 2=zdef compute_grasp_yaw(model, data, gid_target):
    """yaw角度"""
    R = data.geom_xmat[gid_target].reshape(3, 3) #rotation matrix of world frame
    size = model.geom_size[gid_target]   # 半边长 (x,y,z)
...
def simple_robust_grasp_yaw(model, data, gid_target):
    ...
    for axis in range(3):
        ...
        # 计算该轴与地面的平行程度
        horizontalness = 1 - abs(axis_dir[2])  # Z分量越小越水平
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if now - last_print >= PRINT_PERIOD:
                print(f"{phase}  desire_yaw={desired_yaw:.3f}  "
            ...
            if phase == "align_xy":
                ...
                if abs(palm[0]-target[0]) < XY_ALIGN_TOL and abs(palm[1]-target[1]) < XY_ALIGN_TOL:
                   desired_yaw = compute_grasp_yaw(model, data, gid_tgt)
                    # desired_yaw = simple_robust_grasp_yaw(model, data, gid_tgt)
                    # print(f"[GRASP] 计算抓取角度: {np.degrees(desired_yaw):.1f}°")
                   phase = "rotate"
            elif phase == "rotate":
                ...
                if abs(err) < ROT_TOL:
                     print(f"[ROTATE] 目标: {np.degrees(desired_yaw):.1f}°, 当前: {np.degrees(yaw_cur):.1f}°, 误差: {np.degrees(err):.1f}°")
                     phase = "descend"
            elif phase == "descend":
                ...
                data.ctrl[aid_lift] = lift_cmd+0.192
                wait_steps(model, data, 2)
