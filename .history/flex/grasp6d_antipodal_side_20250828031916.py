...
def smooth_normal_at_point(p, pts, Ns, kdt, k=24, sigma=0.005):
    """KNN + 高斯加权 平滑法线，降低三角面量化噪声影响。返回 np.array 或 None。"""
    if k <= 0 or len(pts) == 0:
        return None
...
def main():
    ...
    for si, idx in enumerate(seeds):
        p  = pts[idx]
        n1_raw = unit(Ns[idx])
        n1_sm  = smooth_normal_at_point(p, pts, Ns, kdt,
                                        k=args.avg_norm_k, sigma=args.avg_norm_sigma)
        n1 = n1_sm if (n1_sm is not None and np.isfinite(n1_sm).all()) else n1_raw
        ...
        for j in cand:
            q  = pts[j]
            if (q[2] < args.table_z + args.min_contact_z):
                continue

            n2_raw = unit(Ns[j])
            n2_sm  = smooth_normal_at_point(q, pts, Ns, kdt,
                                            k=args.avg_norm_k, sigma=args.avg_norm_sigma)
            n2 = n2_sm if (n2_sm is not None and np.isfinite(n2_sm).all()) else n2_raw
            ...
            # 摩擦圆锥(带余量)：u ∈ cone(n1, α) 且 -u ∈ cone(n2, α)
            ...
            grasps.append({
                ...
                "normals":  [n1.tolist(), n2.tolist()],    # 两侧法线（已平滑/回退）
