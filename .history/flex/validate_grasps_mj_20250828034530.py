...
# -*- coding: utf-8 -*-
"""
validate_grasps_mp.py
并行验证二指夹爪对单个目标的 6D 抓取（无渲染，支持 MUJOCO_GL=egl）
- 读取 XML（含夹爪+若干LEGO）
- 读取 grasps.json（每个 grasp: position, quaternion_xyzw, width, ...）
- 对指定目标 geom（如 lego_23_geom）逐个尝试抓取
- 成功标准：左右指均接触+合力超过阈值；抬离地面后保持一段时间不掉落
"""
...
def main():
    ...
    ap.add_argument("--xml", required=True, help="MuJoCo 场景 XML（含夹爪+LEGO）")
    ap.add_argument("--target", required=True, help="目标几何名（如 lego_23_geom）")
    ap.add_argument("--grasps", required=True, help="6D 抓取 JSON（数组或 {grasps:[]}）")
