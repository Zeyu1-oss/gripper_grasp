    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if phase == "align_xy":
                ...
                # 当前两指连线角
                phi_cur = finger_line_angle(model, data, bid_left, bid_right)
                # 以“mod π”方式计算误差（平行夹爪对称性）
                err_phi = angle_diff_mod_pi(phi_target, phi_cur)

                # 重要：用“相对角度”映射到 yaw 的绝对目标
                # yaw_des = yaw_cur + err_phi  （因为两指连线绕 z 旋转与yaw改变一一对应）
                yaw_des = wrap_to_pi(yaw_cur + err_phi)
                data.ctrl[aid_rot] = float(np.clip(yaw_des, loR, hiR))
                wait_steps(model, data, ROT_SETTLE_STEPS)

                if abs(err_phi) < ROT_TOL_FINGER:
                    print(f"[ROTATE] 完成! phi_target={np.degrees(phi_target):.1f}°, "
                          f"phi_cur={np.degrees(phi_cur):.1f}°, yaw={np.degrees(yaw_cur):.1f}°")
                    phase = "descend"
