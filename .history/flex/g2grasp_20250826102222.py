...
def contact_normal_force(model, data, i):
    ...
    return abs(f6[0])
def nearest_equiv_yaw(desired, current):
    """在 {desired, desired±π} 里选一个到 current 角差最小的目标，并返回(最佳目标, 误差)。"""
    cand = [wrap_to_pi(desired),
            wrap_to_pi(desired + np.pi),
            wrap_to_pi(desired - np.pi)]
    diffs = [abs(wrap_to_pi(c - current)) for c in cand]
    i = int(np.argmin(diffs))
    best = cand[i]
    err  = wrap_to_pi(best - current)
    return best, err

def short_swing_cmd(current, err, step_max):
    """短摆：把目标设置在 current + 限幅(误差)，防止大跨步/被裁剪。"""
    step = np.clip(err, -step_max, step_max)
    return wrap_to_pi(current + step)
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if phase == "align_xy":
                ...
                # 选最近的等价目标（考虑 ±π）
                best_des, err = nearest_equiv_yaw(desired_yaw, yaw_cur)

                # 短摆步进（位置执行器：把 ctrl 设置成“当前 + 小步长”）
                cmd = short_swing_cmd(yaw_cur, err, ROT_STEP_MAX)
                # 也可以在接近时直接给最终位姿，收敛更快
                if abs(err) < 3*ROT_TOL:
                    cmd = best_des

                # 下发控制（注意：位置执行器的 ctrl 就是目标角度）
                data.ctrl[aid_rot] = float(np.clip(cmd, loR, hiR))
                ...
                # 重新读当前角度
                yaw_cur = float(data.qpos[qadr_yaw])
                _, err = nearest_equiv_yaw(desired_yaw, yaw_cur)
                ...
                if z_err <= 0.005:
                    ...
                    data.ctrl[aid_lift] = lift_cmd+0.199
