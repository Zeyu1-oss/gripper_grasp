...
LIFT_STEP_MAX     = 0.001      # 每步最大 Z 关节改变量 (m)
        ...
        while viewer.is_running():
            ...
            if now - last_print >= PRINT_PERIOD:
                print(f"[{phase:10s}] palm=({palm[0]:+.3f},{palm[1]:+.3f},{palm[2]:.3f}) "
                      f"taget0={target[0]:+.3f} "
                      f"taget1={target[1]:+.3f} " ctrl(right)={data.ctrl[aid_right]:+.3f} "
            ...
            if phase == "align_xy":
                ...
                if abs(palm[0] +  target[0]) < XY_ALIGN_TOL or \
                   abs(palm[1] +  target[0]) < XY_ALIGN_TOL:
