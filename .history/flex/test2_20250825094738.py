...
def compute_grasp_yaw(model, data, gid_tgt, state, longest_axis):
    ...
    if state == "vertical":
        ...
        dir_xy = np.array([long_dir[0], long_dir[1]])
    else:  # horizontal
        # lego Z → palm Y 对齐
        z_dir = axes[2]
    ...
    return np.arctan2(dir_xy[1], dir_xy[0])
    ...
    with mujoco.viewer.launch_passive(model,data) as viewer:
        while viewer.is_running():
            ...
            if phase=="align_xy":
                ...
                # 控制目标设置为 “当前角度 + step”，避免一直撞边界
                data.ctrl[aid_rot] = np.clip(yaw_cur + step, loR, hiR)
                ...
                # 检查收敛
                if abs(err) < ROT_TOL:
                    print(f"[ROTATE] 完成 state={state}, yaw={np.degrees(yaw_cur):.1f}° 目标={np.degrees(desired_yaw):.1f}°")
                    phase = "descend"

                # 兜底：如果 ctrl 已经撞边界还没收敛（只在 horizontal 模式启用）
                if state == "horizontal":
                    if (abs(data.ctrl[aid_rot] - loR) < 1e-3 or abs(data.ctrl[aid_rot] - hiR) < 1e-3) and abs(err) > 0.5:
                        desired_yaw = wrap_to_pi(desired_yaw + np.pi)
                        print(f"[ROTATE] 切换等价目标 yaw={np.degrees(desired_yaw):.1f}° 重新对齐")

            elif phase=="descend":
                tgt_bottom=target_bottom_z(model,data,gid_tgt)
                z_goal=tgt_bottom+DOWN_SAFE_GAP+palm2tip
                err=palm[2]-z_goal
                if err<=0.001: phase="close"
                else:
