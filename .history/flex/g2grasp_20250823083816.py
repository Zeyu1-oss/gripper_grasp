...
def main():
    ...
    loL_cmd, hiL_cmd = model.actuator_ctrlrange[aid_left]
    loR_cmd, hiR_cmd = model.actuator_ctrlrange[aid_right]
    ...
    # 读取当前接触与力
                    ...
                    if not (hitL or hitR):
                        # 还没接触到：两侧一起加紧，微微下压帮助贴合
                        data.ctrl[aid_left]  = CLOSE_CMD
                        data.ctrl[aid_right] = CLOSE_CMD
                        wait_steps(model, data, 2)

                    elif hitL ^ hitR:
                        # 仅一侧接触：继续两侧加紧，并再轻微下压，促成另一侧接触
                        data.ctrl[aid_left]  = CLOSE_CMD-0.5
                        data.ctrl[aid_right] = CLOSE_CMD-0.5
                        data.ctrl[aid_lift]  = float(np.clip(data.ctrl[aid_lift] - 0.0005, loZ, hiZ))
                        wait_steps(model, data, 2)

                    else:
        # 双侧接触：做力均衡
                        diff = leftF - rightF          # >0 说明左边更大
                        # 把力差映射成左右电机指令的微调：左大则减小左指令、增大右指令
                        left_cmd  = np.clip(CLOSE_CMD - K_BAL*diff, loL_cmd, hiL_cmd)
                        right_cmd = np.clip(CLOSE_CMD + K_BAL*diff, loR_cmd, hiR_cmd)

                        data.ctrl[aid_left]  = float(left_cmd)
                        data.ctrl[aid_right] = float(right_cmd)
        # 双侧已接触后不再向下压，避免把目标顶死在托盘上
                        wait_steps(model, data, 2)

        # 满足“力够大 + 力差够小”则进入抬起
                        if (leftF + rightF) >= BOTH_CONTACT_TH and abs(diff) <= F_BAL_TOL:
                            phase = "lift"
