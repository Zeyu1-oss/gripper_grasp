...
def main():
    ...
    open_val_l = lo_l + 0.9*(hi_l - lo_l)
    open_val_r = lo_r + 0.9*(hi_r - lo_r)
    data.ctrl[aid_left]  = open_val_l
    data.ctrl[aid_right] = open_val_r
        ...
        while viewer.is_running():
            ...
            if now - last_print >= PRINT_PERIOD:
                print(f"[{phase:10s}] palm=({palm[0]:+.3f},{palm[1]:+.3f},{palm[2]:.3f}) "
                      ...
                      f "z_goal" #"ctrl(rot)={data.ctrl[aid_rot]:+.3f}")
                last_print = now
            ...
            if phase == "align_xy":
                ...
                data.ctrl[aid_left]  = clamp_to_ctrlrange(model, aid_left,  data.ctrl[aid_left]  - CLOSE_STEP)
                data.ctrl[aid_right] = clamp_to_ctrlrange(model, aid_right, data.ctrl[aid_right] - CLOSE_STEP)
