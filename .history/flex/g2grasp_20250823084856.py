...
def main():
    ...
    # 读取当前接触与力
                    ...
                    elif hitL and hitR:      # 双侧接触 -> 只收紧，不再下降
                        data.ctrl[aid_left]  = np.clip(data.ctrl[aid_left]  + 0.02 * np.sign(CLOSE_CMD), -1, 1)
                        data.ctrl[aid_right] = np.clip(data.ctrl[aid_right] + 0.02 * np.sign(CLOSE_CMD), -1, 1)
        ...
        # 满足“力够大 + 力差够小”则进入抬起
                ...
                step = min(LIFT_UP_VALUE, max(0.0, err) * 0.5)

                data.ctrl[aid_lift]  = float(np.clip(LIFT_UP_VALUE-step, loZ, hiZ))
                data.ctrl[aid_left]  = CLOSE_CMD
                data.ctrl[aid_right] = CLOSE_CMD
                wait_steps(model, data, 2)
