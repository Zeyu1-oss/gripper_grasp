...
def oriented_bbox(mesh: trimesh.Trimesh):
    """
    返回 OBB 尺寸 extents(Ex,Ey,Ez) 以及 OBB->mesh 的 4x4 变换 Tbox2mesh。
    在 OBB 坐标中，盒子中心在原点，轴向为 X/Y/Z。
    """
    extents, Tbox2mesh = trimesh.bounds.oriented_bounds(mesh)
    return np.asarray(extents, float), np.asarray(Tbox2mesh, float)
...
def main():
    ...
    if grasps:
        ...
        print("（注意：这是确定性候选集，不排序；你可按高度、避障、与手爪碰撞等再筛）")
