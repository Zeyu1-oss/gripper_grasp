...
def main():
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            palm   = data.xpos[bid_palm]               # 世界坐标（body）
            target = data.geom_xpos[gid_tgt]           # 目标几何中心（世界）
            ...
            if phase == "align_xy":
                ...
                data.ctrl[aid_lift] = lift_cmd+0.198
                wait_steps(model, data, 2)

                # 进入夹紧：已有接触或高度已经足够近
                if (hitL or hitR) or err <= 0.002:
                    phase = "close"
                ...
                if hitL and hitR and (leftF + rightF) >= BOTH_CONTACT_TH :
                    phase = "lift"
