...
# 旋转控制（按 ctrl 逐步逼近）
ROT_STEP_MAX   = 0.06          # 单步最大推进 (rad)
ROT_TOL        = 0.01          # 与 desired_yaw 的 ctrl 误差阈值 (rad) 判定旋转完成

# 初始与抬起高度（lift 是 position 执行器）
...
# 姿态阈值
VERTICAL_THRESHOLD = 0.7       # |长轴Z分量| > 此值视为站着
...
def classify_vertical_or_horizontal(model, data, gid_target):
    """
    长边 = 局部 z 轴。根据 ez 在世界系的 Z 分量判断站/躺。
    返回: ('vertical' or 'horizontal', R)
    """
    R = data.geom_xmat[gid_target].reshape(3, 3)  # 列为物体局部轴在世界系的方向
    ez = R[:, 2]
...
def compute_desired_yaw_vertical(R):
    """
    站着：lego 的 z 与 palm 的 z 平行，但仍需旋转到使 lego 的 xy 分别平行于 palm 的 xy。
    做法：让 palm 的 x 轴与 lego 的 x 轴在 XY 平面对齐。
      palm x(ψ) = [cosψ, sinψ]
      令 [cosψ, sinψ] 与 ex_xy 对齐 → ψ = atan2(ex_y, ex_x)
    """
    ex = R[:, 0]
...
def compute_desired_yaw_horizontal(R):
    """
    躺着：你要求 lego 的 z 与 palm 的 y 平行。
      palm y(ψ) = [-sinψ, cosψ]
      令 [-sinψ, cosψ] 与 ez_xy 对齐，解得：
        cosψ = ez_y, sinψ = -ez_x → ψ = atan2(-ez_x, ez_y)
    """
    ez = R[:, 2]
    v  = np.array([ez[0], ez[1]])   # ez 的水平投影
    n  = np.linalg.norm(v)
    if n < 1e-6:
        # 极少见退化：直接不转
        return 0.0
...
def compute_desired_yaw(model, data, gid_target):
    """
    返回 (state, desired_yaw)，其中 state ∈ {'vertical','horizontal'}
    """
...
def main():
    ...
    desired_yaw = 0.0
    pose_state = "unknown"   # 'vertical' / 'horizontal'
        ...
        while viewer.is_running():
            ...
            if now - last_print >= PRINT_PERIOD:
                print(f"{phase} state={pose_state} des_yaw={np.degrees(desired_yaw):.1f}° "
                      ...
                      f"err={np.degrees(wrap_to_pi(desired_yaw-data.ctrl[aid_rot])):.1f}°")
                last_print = now
            ...
            if phase == "align_xy":
                ...
                if abs(palm[0]-target[0]) < XY_ALIGN_TOL and abs(palm[1]-target[1]) < XY_ALIGN_TOL:
                    ...
                    phase = "rotate"  # 两种姿态都可能需要转（vertical 也要对齐 xy）

            elif phase == "rotate":
                # 按 ctrl 逐步逼近 desired_yaw
                err_ctrl = wrap_to_pi(desired_yaw - float(data.ctrl[aid_rot]))
                step = float(np.clip(err_ctrl, -ROT_STEP_MAX, ROT_STEP_MAX))
                data.ctrl[aid_rot] = clamp_to_ctrlrange(model, aid_rot, float(data.ctrl[aid_rot]) + step)+0.5
                wait_steps(model, data, 2)

                # 判定完成
                if abs(err_ctrl) < ROT_TOL:
                ...
                # 固持 yaw（避免接触扭回）
                data.ctrl[aid_rot] = clamp_to_ctrlrange(model, aid_rot, desired_yaw)
