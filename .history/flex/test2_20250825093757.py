...
ROT_TOL        = 0.01          # 与 desired_yaw 的实际误差阈值 (rad) 判定旋转完成
...
# 姿态阈值
VERTICAL_THRESHOLD = 0.7       # |ez·Z| > 此值视为站着（长边接近垂直）
...
def compute_desired_yaw_vertical(R):
    ...
    做法：令 palm 的 x 轴与 lego 的 x 轴在 XY 对齐：
        yaw = atan2(ex_y, ex_x)
    """
    ex = R[:, 0]
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if now - last_print >= PRINT_PERIOD:
                err_show = wrap_to_pi(desired_yaw - yaw_cur)
            ...
            if phase == "align_xy":
                ...
                # 用“目标-实际”误差推进 ctrl setpoint，逐步收敛
                err = wrap_to_pi(desired_yaw - yaw_cur)
                step = float(np.clip(err, -ROT_STEP_MAX, ROT_STEP_MAX))
                data.ctrl[aid_rot] = clamp_to_ctrlrange(model, aid_rot, yaw_cur + step)+ pi/2
                ...
                # 固持 yaw（避免被接触扭回）
                yaw_cur = float(data.qpos[qadr_yaw])
                err_hold = wrap_to_pi(desired_yaw - yaw_cur)
