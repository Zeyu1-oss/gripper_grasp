...
def main():
    ...
    if None in (aid_x, aid_y, aid_lift, aid_grasp, aid_rot,
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if phase == "align_xy":
                ...
                if (abs(palm[0] + data.ctrl[aid_x]) > XY_ALIGN_TOL) or (abs(palm[1] + data.ctrl[aid_y]) > XY_ALIGN_TOL:
                ...
                if (abs(data.ctrl[aid_grasp] - lo_g) < 1e-6 or abs(data.ctrl[aid_grasp] - hi_g) < 1e-6) \
                   and (not hitL and not hitR):
