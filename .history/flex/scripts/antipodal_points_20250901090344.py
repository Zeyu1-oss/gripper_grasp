...
class AntipodalGraspGenerator:
    def __init__(self, mu=0.4, opp_angle_deg=30.0, m_dirs=32, seed=42,
    ...
    def _contact_wrench_rays(self, p, n, com, torque_scale):
        """
        返回 6×m_dirs 的矩阵，每列 [f; τ]，τ=(p-com)×f / torque_scale
        使用近似摩擦锥基：f = normalize(n + mu * t)，t 在切平面离散一圈
        """
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        for i in tqdm(range(len(points)), desc=f"候选生成 {os.path.basename(mesh_path)}"):
            ...
            p2 = locs[k_far]
            n2 = normals[i]  # 用 tri_ids[k_far] 也可：mesh.face_normals[tri_ids[k_far]]
            ...
            if (width < wmin) or (width > wmax):     # 修正逻辑
                continue
...
# ========== CLI ==========
def main():
    ap = argparse.ArgumentParser("Antipodal grasp generator (均匀覆盖 + 摩擦锥 ε)")
    ...
    gen = AntipodalGraspGenerator()  # 保持你的默认参数
