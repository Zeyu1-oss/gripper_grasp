...
def _epsilon_qp(W):
    ...
    if res.success:
        lam = res.x
        return float(np.linalg.norm(W@lam)), True
    # fallback NNLS
    rho=10.0
    A = np.vstack([W, np.sqrt(rho)*np.ones((1,m))])
    b = np.zeros(A.shape[0]); b[-1]=np.sqrt(rho)
    lam, _ = nnls(A,b)
    if np.sum(lam)>1e-12: lam/=np.sum(lam)
    return float(np.linalg.norm(W@lam)), False

def wrench_quality(p1,p2,n1,n2,mesh,mu=0.6,m_dirs=8):
    com = mesh.center_mass
    Lc = np.linalg.norm(mesh.extents)
    W1 = _contact_wrench_rays(p1,n1,mu,m_dirs,com,Lc)
    W2 = _contact_wrench_rays(p2,n2,mu,m_dirs,com,Lc)
    W = np.concatenate([W1,W2],axis=1)
    eps,_ = _epsilon_qp(W)
    return 1.0/(eps+1e-6)

# ================= 抓取点 → 6D pose =================
def pair_to_pose(p1,p2,roll_deg=0.0):
    c = (p1+p2)/2.0
    x = (p2-p1); x/=np.linalg.norm(x)
    up = np.array([0.,0.,1.]) if abs(np.dot(x,[0,0,1]))<0.95 else np.array([0.,1.,0.])
    y0 = _unit(np.cross(up,x))
    z0 = np.cross(x,y0)
    phi=np.deg2rad(roll_deg)
    y=np.cos(phi)*y0+np.sin(phi)*z0
    z=-np.sin(phi)*y0+np.cos(phi)*z0
    Rm=np.column_stack([x,y,z])
    quat=R.from_matrix(Rm).as_quat()  # [x,y,z,w]
    return c, quat

# ================= 主流程 =================
def generate_side_grasps(mesh, mu=0.6, num_samples=20000,
                         wmin=None,wmax=None,
                         rays_per_point=8,cone_half_deg=12,
                         z_cutoff=0.02, topk=200):
    ...
    zmin=mesh.bounds[0,2]  # 世界坐标 z 最小值
    candidates=[]

    for i in trange(num_samples,desc="采样"):
        p1=points[i]; n1=_unit(normals[i])
        # ---- 排除底面点 ----
        bottom_ration = 0.05
        zmin, zmax = float(mesh.bounds[mesh.bounds[0,2]]).float(mesh.bounds[1,2])
        height = zmax- zmin 
        bottom_margin= zmin+ bottom_ration*height
        if pi[2] <= bottom_margin
            continue
        axis=-n1
        t1,t2=_orthonormal_tangent_basis(axis)
        phis=rng.uniform(0,2*np.pi,size=rays_per_point)
        dirs=[_unit(axis*np.cos(theta)+(np.cos(pshi)*t1+np.sin(phi)*t2)*np.sin(theta)) for phi in phis]
        ...
        k=np.argmax(dists); p2=locs[k]; n2=_unit(mesh.face_normals[tri_ids[k]])
        w=np.linalg.norm(p2-p1)
        if w<wmin or w>wmax: continue
        if not is_antipodal(p1,n1,p2,n2,mu): continue
        quality=wrench_quality(p1,p2,n1,n2,mesh,mu=mu)
        candidates.append((quality,p1,p2,n1,n2))
    # 排序取前 K
...
if __name__=="__main__":
    results_dir=os.path.abspath(os.path.join(os.path.dirname(__file__),"../results/lego_grasps"))
    os.makedirs(results_dir,exist_ok=True)
    mesh_path=os.path.abspath(os.path.join(os.path.dirname(__file__),"../lego.obj"))
    mesh=trimesh.load(mesh_path,force='mesh')
    mesh.apply_scale(0.01)
    print(f"Loaded mesh: {mesh_path}, verts={len(mesh.vertices)}, faces={len(mesh.faces)}")

    grasps=generate_side_grasps(mesh,mu=0.6,num_samples=30000,topk=200)
    print(f"✅ 生成 {len(grasps)} 个候选抓取（仅侧边）")

    # 转为 6D pose 并保存
    poses=[]
    for q,p1,p2,n1,n2 in grasps:
        c,quat=pair_to_pose(p1,p2,roll_deg=0.0)
        poses.append((c,quat,q))
    save_path=os.path.join(results_dir,"lego_grasps.npy")
    np.save(save_path,poses)
    print(f"✅ 已保存到 {save_path} (内容: (center xyz, quat xyzw, quality))")
# import numpy as np
# import trimesh
# import os
# from tqdm import trange
