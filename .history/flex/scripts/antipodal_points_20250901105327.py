...
class AntipodalGraspGenerator:
    def __init__(self, mu=0.4, opp_angle_deg=30.0, m_dirs=8, seed=42,
        ...
        self.m_dirs = m_dirs          # 摩擦锥离散方向数（可调 8/16/32）
    ...
    def _geom_score_pair(self, p1, p2, n1, n2, wmin, wmax):
        ...
        return margin + 0.3*anti - 0.1*w_pen

    # ---------- 接触摩擦锥 -> 力-力矩列向量 ----------
    def _contact_wrench_rays(self, p, n, com, torque_scale):
        t1, t2 = self._orthonormal_tangent_basis(n)
        r = p - com
        cols = []
        for k in range(self.m_dirs):
            phi = 2*np.pi*k/self.m_dirs
            t = np.cos(phi)*t1 + np.sin(phi)*t2
            f = self._unit(n + self.mu*t)       # 近似 PCWF
    ...
    def _epsilon_qp(self, W):
        ...
        # 退化兜底
        lam, _ = nnls(W, np.zeros(W.shape[0]))
        s = np.sum(lam)
        if s > 1e-12: lam = lam/s
        return float(np.linalg.norm(W @ lam))

    # ---------- 侧面优先的表面采样 ----------
    def _side_biased_surface_samples(self, mesh, num_samples,
                                     z_crop_ratio: float = 0.05,
                                     side_bias_alpha: float = 2.0):
        """
        - 按面面积 * (1 - |n·ẑ|)^alpha 做权重（越接近侧面权重越大）
        - 裁掉 z 的上下 z_crop_ratio 区间
        """
        V = mesh.vertices
        F = mesh.faces
        ...
        z_lo = zmin + z_crop_ratio * (zmax - zmin)
        z_hi = zmax - z_crop_ratio * (zmax - zmin)

        zhat = np.array([0.,0.,1.], float)
        side_weight = (1.0 - np.abs(FN @ zhat)) ** side_bias_alpha  # 侧面更大
        band_mask = (FC[:,2] >= z_lo) & (FC[:,2] <= z_hi)           # 去掉最上/最下

        w = FA * side_weight * band_mask.astype(float)
        if np.all(w <= 0):
            # 极端退化时，退回到纯面积权重
            w = FA.copy()

        p = w / (w.sum() + 1e-12)
        idx = self.rng.choice(len(F), size=num_samples, replace=True, p=p)

        # 在每个三角形内做一次均匀重心采样
        tri = V[F[idx]]
        r1 = self.rng.random(num_samples)
        r2 = self.rng.random(num_samples)
        s1 = 1.0 - np.sqrt(r1)
        s2 = np.sqrt(r1) * (1.0 - r2)
        s3 = np.sqrt(r1) * r2
        pts = (tri[:,0]*s1[:,None] + tri[:,1]*s2[:,None] + tri[:,2]*s3[:,None])
        nrm = FN[idx]
        return pts.astype(float), nrm.astype(float)

    # ---------- PCA / Diversity ----------
    def _make_feature(self, c):
        """
        组装 PCA 用特征：
        [center(3), 夹爪方向 u(3), 法向合向 s(3), width(1)] -> 10D
        """
        p1 = c["p1"]; p2 = c["p2"]; n1 = c["n1"]; n2 = c["n2"]
        center = 0.5*(p1+p2)
        u = self._unit(p2 - p1)
        s = self._unit(n1 + n2)
        width = np.array([c["width"]], float)
        return np.concatenate([center, u, s, width], axis=0)

    def _pca_fit_transform(self, X, k=3):
        """
        返回：Z(n,k)、components(Vt)、explained_ratio
        """
        ...
        # 解释方差比
        var = (S**2) / max(n-1, 1)
        explained_ratio = var / (var.sum() + 1e-12)
        k = int(min(k, Vt.shape[0]))
        Z = Xn @ Vt[:k].T
        return Z, Vt[:k], explained_ratio[:k], mu, std

    def _diversity_stats(self, Z):
        """
        在 PCA 空间里做简单多样性度量：
        - pairwise 距离均值/最小/最大（在样本数<=1500时计算）
        - 平均最近邻距离
        """
        n = Z.shape[0]
        if n == 0:
            return {"pair_mean": 0.0, "pair_min": 0.0, "pair_max": 0.0, "nn_mean": 0.0}
        ...
        for i in range(n):
            ...
            d[i] = np.inf
            nn_d.append(np.min(d))
        nn_d = np.asarray(nn_d)
        nn_mean = float(np.mean(nn_d))

        # pairwise 距离（限制 n<=1500 避免 O(n^2) 太重）
        if n <= 1500:
            # 利用矩阵技巧：||a-b||^2 = ||a||^2 + ||b||^2 - 2a·b
            G = Z @ Z.T
            nrm2 = np.sum(Z*Z, axis=1, keepdims=True)
            D2 = nrm2 + nrm2.T - 2*G
            D = np.sqrt(np.maximum(D2, 0.0))
            iu = np.triu_indices(n, 1)
            pair = D[iu]
            pair_mean = float(np.mean(pair))
            pair_min  = float(np.min(pair)) if pair.size>0 else 0.0
            pair_max  = float(np.max(pair)) if pair.size>0 else 0.0
        else:
            pair_mean = pair_min = pair_max = 0.0

        return {"pair_mean": pair_mean, "pair_min": pair_min,
                "pair_max": pair_max, "nn_mean": nn_mean}

    def _fps_on_pcs(self, Z, scores, k):
        """
        在 PCA 空间 Z(n,d) 上做最远点采样（Farthest Point Sampling）
        - 以 scores 最大者为初始种子（例如 eps）
        - 迭代选择与当前集合距离最大的点
        返回选中索引列表
        """
        n = Z.shape[0]
        ...
        selected = [seed]
        # 维护每个点到已选集合的最小距离
        d2 = np.sum((Z - Z[seed])**2, axis=1)
        for _ in range(1, k):
            # 下一个是“最远”的点
            idx = int(np.argmax(d2))
            selected.append(idx)
            # 更新所有点到集合的最小距离
            d2 = np.minimum(d2, np.sum((Z - Z[idx])**2, axis=1))
        return selected
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
                          ...
                          out_path=None,
                          # diversity 相关
                          diverse_select=False, eps_min=0.0, pca_k=3,
                          # 侧面采样参数
                          z_crop_ratio=0.05, side_bias_alpha=2.0):
        ...
        # 侧面优先采样
        points, normals = self._side_biased_surface_samples(
            mesh, num_samples, z_crop_ratio=z_crop_ratio, side_bias_alpha=side_bias_alpha
        )
        ...
        for i in tqdm(range(len(points)), desc=f"候选生成 {os.path.basename(mesh_path)}"):
            ...
            k_far = int(np.argmax(dists))
            p2 = locs[k_far]
            n2 = mesh.face_normals[tri_ids[k_far]]
            ...
            W1 = self._contact_wrench_rays(p1, n1, com, Lc)
            W2 = self._contact_wrench_rays(p2, n2, com, Lc)
        ...
        if cand.size == 0:
            print("[EPS 统计] 没有通过几何/COM筛选的候选。")
            if out_path:
                os.makedirs(os.path.dirname(out_path), exist_ok=True)
                np.save(out_path, cand, allow_pickle=True)
                print(f"✅ 保存 0 条抓取到 {out_path}")
            return []

        # 先做 ε >= eps_min 的过滤（你说 ε>0 就 OK，默认 eps_min=0）
        mask = np.array([c["eps"] >= eps_min for c in cand], dtype=bool)
        filt = cand[mask]
        if filt.size == 0:
            print(f"[EPS 统计] 所有候选的 ε 都 < {eps_min}。")
            if out_path:
                os.makedirs(os.path.dirname(out_path), exist_ok=True)
                np.save(out_path, np.array([], dtype=object), allow_pickle=True)
                print(f"✅ 保存 0 条抓取到 {out_path}")
            return []

        # --- 多样性：PCA 统计 ---
        X = np.stack([self._make_feature(c) for c in filt], axis=0)
        Z, comps, ratio, mu, std = self._pca_fit_transform(X, k=pca_k)
        print("[PCA] explained variance ratio (前 {} PCs): {}".format(
            pca_k, ", ".join([f"{r:.3f}" for r in ratio])
        ))
        div_all = self._diversity_stats(Z)
        print("[Diversity@ALL] pair_mean={:.4f}, pair_min={:.4f}, pair_max={:.4f}, nn_mean={:.4f}".format(
            div_all["pair_mean"], div_all["pair_min"], div_all["pair_max"], div_all["nn_mean"]
        ))

        # --- 最终选择 ---
        if diverse_select:
            # 用 FPS 在 PCA 空间里选出更分散的 topk。评分用 eps 做初始种子。
            eps_scores = np.array([c["eps"] for c in filt], float)
            sel_idx = self._fps_on_pcs(Z, eps_scores, k=min(topk, len(filt)))
            results = [filt[i].items() for i in sel_idx]
        else:
            # 按 eps 降序取前 topk（保持你之前的默认）
            order = np.argsort([-c["eps"] for c in filt])
            order = order[:min(topk, len(filt))]
            results = [filt[i].items() for i in order]

        # 终集多样性统计（在选出的集合上再报一次）
        X_fin = np.stack([self._make_feature(c) for c in results], axis=0)
        # 用同一均值/方差和同一投影（与上面一致）
        Xn_fin = (X_fin - mu) / (std + 1e-12)
        Z_fin = Xn_fin @ comps.T
        div_fin = self._diversity_stats(Z_fin)
        print("[Diversity@FINAL] pair_mean={:.4f}, pair_min={:.4f}, pair_max={:.4f}, nn_mean={:.4f}".format(
            div_fin["pair_mean"], div_fin["pair_min"], div_fin["pair_max"], div_fin["nn_mean"]
        ))

        # ε 统计
        eps_vals = np.array([c["eps"] for c in results], float)
        print(f"[EPS 统计] mean={eps_vals.mean():.4e}, max={eps_vals.max():.4e}, "
              f"min={eps_vals.min():.4e}, n={len(results)}")
...
# ========== CLI ==========
def main():
    ap = argparse.ArgumentParser("Antipodal grasp generator (侧面优先 + 摩擦锥 ε + PCA Diversity)")
    ...
    # 多样性与阈值
    ap.add_argument("--diverse_select", action="store_true", help="在 PCA 空间做最远点采样以提升多样性")
    ap.add_argument("--eps_min", type=float, default=0.0, help="过滤阈值：仅保留 epsilon >= 该值的候选")
    ap.add_argument("--pca_k", type=int, default=3, help="用于多样性度量/选择的主成分数")

    # 侧面采样控制
    ap.add_argument("--z_crop_ratio", type=float, default=0.05, help="裁掉上下端 z 比例")
    ap.add_argument("--side_bias_alpha", type=float, default=2.0, help="侧面权重幂指数，越大越偏向侧面")
    ...
    for mesh_path in args.meshes:
        ...
        gen.generate_for_mesh(mesh_path,
                              ...
                              topk=args.topk,
                              out_path=out_path,
                              diverse_select=args.diverse_select,
                              eps_min=args.eps_min,
                              pca_k=args.pca_k,
                              z_crop_ratio=args.z_crop_ratio,
                              side_bias_alpha=args.side_bias_alpha)
