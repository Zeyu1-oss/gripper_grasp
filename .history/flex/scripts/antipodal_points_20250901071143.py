...
class AntipodalGraspGenerator:
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        for i in trange(num_samples, desc=f"候选 {os.path.basename(mesh_path)}"):
            ...
            # 过滤 X/Y：只要边缘
            edge_tol_x = self.edge_ratio * (xmax - xmin)
            edge_tol_y = self.edge_ratio * (ymax - ymin)
            is_edge_x = (abs(p1[0]-xmin) < edge_tol_x) or (abs(p1[0]-xmax) < edge_tol_x)
            is_edge_y = (abs(p1[1]-ymin) < edge_tol_y) or (abs(p1[1]-ymax) < edge_tol_y)
            if not (is_edge_x or is_edge_y):
                continue
...
# ========== main ==========
def main():
    ap = argparse.ArgumentParser("Batch antipodal grasp generator (with edge sampling)")
    ...
    ap.add_argument("--edge_ratio", type=float, default=0.1, help="X/Y 边缘比例 (默认 0.1)")
    args = ap.parse_args()
