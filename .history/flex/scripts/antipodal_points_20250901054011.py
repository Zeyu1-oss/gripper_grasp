...
class AntipodalGraspGenerator:
    def __init__(self,
                 ...
                 bottom_ratio=0.05,
                 top_ratio=0.05,
                 ray_mode="normal",   # "normal" or "cone"
                 rays_per_point=10,
                 cone_half_deg=12.0,
                 nearest_exit=True):
        self.mu = mu
    ...
    #
    # 其中：
    #   - W ∈ R^(6×m)：接触力-力矩矩阵，每列是一个接触方向的广义力向量 [f; τ]
    #   - λ ∈ R^m：非负权重，表示在摩擦锥基底上的凸组合
        m = W.shape[1]
        H = W.T @ W
    ...
    # 步骤：
    #   1. 确定质心 com：
    #        - 若 mesh watertight，用 mesh.center_mass
    #        - 否则退化成包围盒中心 (bounds 平均)
    #
    #   2. 计算每个接触点的离散摩擦锥：
    #        - 使用 _contact_wrench_rays()
    #        - 每个摩擦锥展开成 m_dirs 个方向
    #        - 每列向量包含 [f; τ] (力 + 力矩)，τ = (p - com) × f
    #
    #   3. 拼接两个接触点的 W 矩阵：
    #        W ∈ R^(6×(2*m_dirs))
    #
    ...
    #
    #   5. 返回：
    #        eps  = ε （epsilon metric）
    #        wscore = 1 / (ε + 1e-6) （作为惩罚项，越小越好）
    #
    # 几何意义：
    #   - eps 越大，力闭合越稳健
    #   - wscore 用于综合排序时拉开分数差距
    # ------------------------------------------------------------
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        side_zmin = zmin + self.bottom_ratio*(zmax-zmin)
        side_zmax = zmax - self.top_ratio*(zmax-zmin)
        ...
        pairs=[]
        for r in results:
            center, R, quat = self._pair_to_pose(r.p1,r.p2,r.n1,r.n2)
            pairs.append({"p1":r.p1,"p2":r.p2,"n1":r.n1,"n2":r.n2,
                          "center":center,"R":R,"quat_wxyz":quat})
...
# ========== main ==========
def main():
    ap = argparse.ArgumentParser("Batch antipodal grasp generator (class-based)")
    ...
    gen = AntipodalGraspGenerator(mu=0.4, opp_angle_deg=30.0, m_dirs=8, seed=42)
