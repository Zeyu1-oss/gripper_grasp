...
# 关指策略参数（都不会让 opening=0）
OPENING_OPEN   = 0.03     # m，初始开口总和（= left + right）
OPENING_MIN    = 0.0015   # m，最小安全开口，绝不低于它
CLOSE_TIME     = 0.8      # s，期望关指时长（用于步进大小）
FPS            = 400
CLOSE_STEP_MAX = 0.0003   # m/步，单步最大收缩量（越小越不穿模）
BACKOFF_STEP   = 0.0006   # m，检测到渗透回退量

# 接触判定
FORCE_TH_EACH  = 0.4      # N，每个手指的法向力阈值
PEN_THRESH     = 0.0004   # m，允许的渗透上限（超过则回退）
...
def yaw_from_R(Rm):
    return float(np.arctan2(Rm[1,0], Rm[0,0]))  # 仅 yaw 自由度
...
def measure_finger_contacts(model, data):
    """返回：lF, rF, minPenL, minPenR（渗透>0取0，渗透<0取其绝对值）"""
...
def run():
    ...
    # 无重力初始化；手有 gravcomp=1，后续开重力也不会下坠
    model.opt.gravity[:] = [0,0,0]
    mujoco.mj_resetData(model, data)

    # 直接把手与开口写进 qpos（不用 ctrl）
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        print("阶段 4：开重力（手 gravcomp，不受重力）；观察 LEGO …")
