...
def key_callback(key):
    global key_states
    key_states[key] = True  # 标记按键被按下
    return False  # 让查看器继续处理其他事件
...
if __name__ == "__main__":
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
        ...
        while viewer.is_running():
            # 处理按键输入
            if mujoco.mjtKey.mjKEY_SPACE in key_states:
                gravity_enabled = not gravity_enabled
                set_gravity(model, gravity_enabled)
                print(f"重力 {'开启' if gravity_enabled else '关闭'}")
                key_states.pop(mujoco.mjtKey.mjKEY_SPACE, None)
            
            if mujoco.mjtKey.mjKEY_G in key_states and not grab_started:
                print("开始抓取：闭合夹爪")
                set_gripper_opening(data, opening=0.0)
                grab_started = True
                key_states.pop(mujoco.mjtKey.mjKEY_G, None)
            
            if mujoco.mjtKey.mjKEY_ESC in key_states:
                print("用户退出仿真")
                break
            ...
            if gravity_enabled and grab_started:
                ...
                if data.time > 2.0:
                    final_displacement = np.linalg.norm(lego_pos_current - lego_pos_before)
                    print(f"仿真结束，LEGO位移: {final_displacement:.4f} 米")
