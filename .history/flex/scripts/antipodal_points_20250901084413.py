...
class AntipodalGraspGenerator:
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        # 打印统计
        eps_vals = [c["eps"] for c in results]
        if len(eps_vals) > 0:
            print(f"[EPS 统计] 平均 = {np.mean(eps_vals):.4f}, "
                  f"最大 = {np.max(eps_vals):.4f}, 最小 = {np.min(eps_vals):.4f}")
...
# ========== main ==========
def main():
    ap = argparse.ArgumentParser("Antipodal grasp generator (摩擦锥 ε-metric)")
