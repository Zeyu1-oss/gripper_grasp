...
def main():
    ...
    if min(bid_left, bid_right, bid_lego, bid_root) < 0:
        raise RuntimeError("找不到必要的body")

    # 调试输出：验证位置对齐
    print(f"📐 抓取中心: {center}")
    print(f"📍 Mocap位置: {data.mocap_pos[mocapid]}")
    print(f"📍 Root位置: {data.xpos[bid_root]}")
    print(f"📍 LEGO位置: {data.xpos[bid_lego]}")
    print(f"📏 位置误差: {np.linalg.norm(data.mocap_pos[mocapid] - data.xpos[bid_root])}")
