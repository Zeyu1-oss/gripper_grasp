...
# 逐步闭合（不写 0）
OPENING_OPEN   = 0.03     # m，总开口（= left + right）
OPENING_MIN    = 0.0015   # m，最小安全开口
CLOSE_TIME     = 0.8      # s
FPS            = 800
CLOSE_STEP_MAX = 0.0002   # m/步
BACKOFF_STEP   = 0.0005   # m

# 接触/渗透阈值
FORCE_TH_EACH  = 0.4      # N（每侧）
PEN_THRESH     = 0.0004   # m
...
def measure_finger_contacts(model, data):
    ...
    for i in range(data.ncon):
        ...
        pen = max(0.0, -float(c.dist))  # dist<0 为渗透
...
def main():
    print("阶段 0：加载模型与姿态（不使用 ctrl）…")
    model = mujoco.MjModel.from_xml_path(XML_PATH)
    data  = mujoco.MjData(model)

    # 完全禁用执行器，避免任何 ctrl 影响
    try:
    ...
    q_wxyz = to_mujoco_quat(Rm)

    # 找到 mocap 索引（只有一个 mocap）
    if model.nmocap != 1:
        raise RuntimeError(f"XML 里应有 1 个 mocap（现在 nmocap={model.nmocap}）")
    # 把 mocap 放到 6D 位姿（焊接会把 palm 刚性绑定到这里）
    data.mocap_pos[0]  = center
    data.mocap_quat[0] = q_wxyz
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        print("阶段 4：开启重力（手被焊接不动），观察 LEGO …")
