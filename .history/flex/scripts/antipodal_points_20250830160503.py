...
def generate_grasp_pairs_with_force_closure(
    ...
    bottom_ratio=0.05,          # ⬅⬅ 新增：屏蔽底部 5% 高度
    top_ratio=0.05,             # ⬅⬅ 新增：屏蔽顶部 5% 高度
    seed=0
):
    ...
    for i in trange(num_samples, desc="生成候选(射线)"):
        ...
        # 建议：最近命中（watertight 稳定）；否则最远命中+厚度阈值
        locs, idx_ray, tri_ids = ray_engine.intersects_location(
        ...
        for rid, hit_list in by_ray.items():
            ...
            if USE_EXIT_NEAREST:
                ...
                t_min = max(5.0*eps, 0.25*wmin)  # 自适应最小厚度
                mask = dists >= t_min
                if not np.any(mask):
                    continue
    ...
    return results[:topk]# =================== 脚本入口（只保存点对，不改你的保存类型） ===================
