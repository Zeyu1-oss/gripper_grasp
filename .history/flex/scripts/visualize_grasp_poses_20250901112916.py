...
def rot_to_euler_zyx(R):
    """3x3 -> (yaw(Z), pitch(Y), roll(X)) in radians"""
    R = np.asarray(R, float)
    sy = -R[2,0]
...
def draw_two_parallel_fingers(ax, center, R, width, finger_len=0.04,
    ...
    """在 palm 原点沿 z 方向画两根平行线；两线在 x 轴两侧，间距=width"""
...
def plot_grasps(npy_path, mesh_path, indices=None,
    ...
    for i in picks:
        ...
        ypr = rot_to_euler_zyx(R)  # (yaw, pitch, roll)
    ...
    ax.set_title("6D poses + two parallel finger lines")
    ax.set_xlabel("X"); ax.set_ylabel("Y"); ax.set_zlabel("Z")
    plt.tight_layout()
    if save:
        plt.savefig(save, dpi=200); print(f"已保存图片: {save}")
    plt.show()

# ========== CLI ==========
def build_argparser():
    ap = argparse.ArgumentParser("显示 6D 位姿 + 两根手指线")
