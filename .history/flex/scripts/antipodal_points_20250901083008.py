...
class AntipodalGraspGenerator:
    ...
    def _geom_score_pair(self, p1, p2, n1, n2, wmin, wmax):
        v, d = p2 - p1, np.linalg.norm(p2 - p1)
        ...
        if d < wmin: w_pen = (wmin - d)**2
            ...
            mid = 0.5*(wmin + wmax)
            denom = 0.5*(wmax - wmin) + 1e-9
            w_pen = 0.25*((d - mid)/denom)**2
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        wmin, wmax = 0.008, 0.05
        cos_opp = np.cos(np.deg2rad(self.opp_angle_deg))
        eps_offset = 1e-5 * np.linalg.norm(mesh.extents)
        com = mesh.center_mass if mesh.is_watertight else mesh.centroid

        pts, norms = self._uniform_surface_samples(mesh, num_samples)
        ray = trimesh.ray.ray_triangle.RayMeshIntersector(mesh)

        cand = []
        for p1, n1 in tqdm(zip(pts, norms), total=len(pts), desc="候选生成"):
            origin, dir = p1 - eps_offset * n1, -n1 / np.linalg.norm(n1)
            locs, _, tids = ray.intersects_location(np.array([origin]), np.array([dir]), multiple_hits=True)
            if len(locs) == 0: continue
            dists = np.linalg.norm(locs - p1, axis=1)
            k = np.argmax(dists)
            p2 = locs[k]; n2 = mesh.face_normals[tids[k]]
            ...
            if self.require_through_com:
                u = (p2 - p1) / (width + 1e-12)
                dist_line = np.linalg.norm(np.cross(u, com - p1))
                if dist_line > self.through_tol_ratio * max(ex, ey, ez):
                    continue
            gscore = self._geom_score_pair(p1, p2, n1, n2, wmin, wmax)
            eps = self._wrench_quality(p1, p2, n1, n2, mesh)
            cand.append({"p1": p1, "p2": p2, "gscore": gscore, "eps": eps,
                         "width": width, "center": 0.5*(p1+p2)})

        # 阈值过滤 & 随机选择
        eps_thresh = 1e-3
        filt = [c for c in cand if c["eps"] > eps_thresh]
        self.rng.shuffle(filt)
        results = filt[:topk]
