...
# ==================== 可调参数 ====================
TORQUE_OPEN  =  0.5
TORQUE_CLOSE = -0.5            # 若握不住，可把 XML motor ctrlrange 改到 -5 5，并把此值加到 -1.5 ~ -2.0
...
def write_lego_unit(lego_mesh_path, template_path):
    """可选：在运行前自动重写 lego_unit.xml 的 mesh 路径（mesh 接触，最宽容）。"""
  ...
  <worldbody>
    <body name="lego" pos="0 0 0">
      ...
      <geom type="mesh" name="lego_01" mesh="lego" material="mat_green" friction="2.0 0.01 0.00001"/>
...
def dump_gripper_pose(model, data, mocapid):
    ...
    palm_q_xyzw = R.from_matrix(palm_R).as_quat()
    palm_q_wxyz = np.array([palm_q_xyzw[3], *palm_q_xyzw[:3]])
...
def enforce_palm_pose(model, data, center, Rm):
    """优先等 weld 拉齐；如误差仍大，则直接写 freejoint 的 qpos 兜底。"""
    bid_palm = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "palm")
    # 给 solver 一点时间（如果 weld 正常，这里应很快对齐）
    for _ in range(10):
        mujoco.mj_step(model, data)
    # 误差评估
    pos_err = float(np.linalg.norm(data.xpos[bid_palm] - center))
    palm_R  = data.xmat[bid_palm].reshape(3,3)
    ang_err = float(np.rad2deg((R.from_matrix(palm_R) * R.from_matrix(Rm).inv()).magnitude()))
    if pos_err > 1e-3 or ang_err > 0.5:
        adr = jadr(model, "palm_free")  # 需要 <freejoint name="palm_free"/>
        data.qpos[adr:adr+3]   = center
        data.qpos[adr+3:adr+7] = to_mj_quat_from_R(Rm)  # [w,x,y,z]
...
def run_once(hand_xml, poses_path, pose_idx, use_viewer=True):
    ...
    print(f"Model: {getattr(model, 'name', '(unknown)')}, equalities: {model.ne}")
    ...
    # 强制对齐（weld 优先，不行则改 freejoint qpos）
    enforce_palm_pose(model, data, center, Rm)
...
def main():
    ...
    # 可选：自动把 lego_unit.xml 指向目标网格（mesh 接触）
