...
class AntipodalGraspGenerator:
    ...
    def _contact_wrench_rays(self, p, n, com, torque_scale):
        ...
        for k in range(self.m_dirs):
            ...
            f = self._unit(n + self.mu*t)           # 离散摩擦锥方向（单位力）
            tau = np.cross(r, f) / (torque_scale + 1e-12)  # 力矩按物体尺度归一化
            cols.append(np.hstack([f, tau]))
        return np.column_stack(cols)                 # 6×m_dirs

    # ========= 旧的“距离凸包”QP（仅兜底用）=========
    def _epsilon_qp_min_dist(self, W):
        # 注意：这个值对“力闭合”会接近 0，仅作兜底不推荐当 ε-metric
        m = W.shape[1]
        H = W.T @ W
    ...
    # ========= Ferrari–Canny ε（凸包面距离法，推荐）=========
    def _epsilon_fc(self, W):
        """
        真正的 ε-metric：若原点在凸包内，ε = 原点到各内向面的最小距离；
        若原点不在凸包内，ε = 0。
        SciPy ConvexHull: equations 行为 A x + b == 0，且 A x + b <= 0 为“在凸包内”。
        """
        P = W.T  # (m, 6)
        try:
            hull = ConvexHull(P)
            A = hull.equations[:, :-1]  # (num_facets, 6)
            b = hull.equations[:, -1]   # (num_facets,)
            # 原点是否在凸包内：对 x=0，有 A*0 + b = b，需要 b <= 0
            if np.any(b > 1e-12):
                return 0.0
            # 到每个面的距离： -b / ||A||
            denom = np.linalg.norm(A, axis=1) + 1e-12
            dists = -b / denom
            eps = float(np.max([0.0, np.min(dists)]))
            return eps
        except QhullError:
            # 点退化或共面等情况，退回到 QP 版本（会很小）
            return self._epsilon_qp_min_dist(W)
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        for i in tqdm(range(len(points)), desc=f"候选生成 {os.path.basename(mesh_path)}"):
            ...
            W1 = self._contact_wrench_rays(p1, n1, com, Lc)   # 6×m_dirs
            W2 = self._contact_wrench_rays(p2, n2, com, Lc)   # 6×m_dirs
            W  = np.concatenate([W1, W2], axis=1)             # 6×(2*m_dirs)

            gscore = self._geom_score_pair(p1,p2,n1,n2,wmin,wmax)
            eps    = self._epsilon_fc(W)   # ★ 使用凸包面距离的真 ε
        ...
        # 阈值 + 打乱 + 取前 topk
        eps_thresh = 1e-14
        filtered = [c for c in candidates if c["eps"] > eps_thresh]
        ...
        if len(results) > 0:
            ...
            print(f"[EPS 统计] 平均 = {eps_vals.mean():.4f}, "
                  f"最大 = {eps_vals.max():.4f}, 最小 = {eps_vals.min():.4f}, N={len(eps_vals)}")
        else:
            print("[EPS 统计] 无有效抓取（阈值或几何条件过严）")
...
# ========== main ==========
def main():
    ap = argparse.ArgumentParser("Antipodal grasp generator (摩擦锥 ε-metric: 凸包面距离)")
