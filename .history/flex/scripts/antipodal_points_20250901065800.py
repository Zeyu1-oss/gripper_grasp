...
class AntipodalGraspGenerator:
    ...
    def _wrench_quality(self, p1, p2, n1, n2, mesh):
    ...
    def _sample_uniform_sides(self, mesh, num_samples):
        # 多采样一点
        points, face_idx = trimesh.sample.sample_surface(mesh, num_samples*3)
        normals = mesh.face_normals[face_idx]

        # 分类: 根据法向绝对值的主导分量
        mask_x = (np.abs(normals[:,0]) >= np.abs(normals[:,1])) & (np.abs(normals[:,0]) >= np.abs(normals[:,2]))
        mask_y = (np.abs(normals[:,1]) >  np.abs(normals[:,0])) & (np.abs(normals[:,1]) >= np.abs(normals[:,2]))
        mask_z = (np.abs(normals[:,2]) >  np.abs(normals[:,0])) & (np.abs(normals[:,2]) >  np.abs(normals[:,1]))

        # 从三类里均匀抽样
        nx = ny = nz = num_samples // 3
        px, nxm = points[mask_x], normals[mask_x]
        py, nym = points[mask_y], normals[mask_y]
        pz, nzm = points[mask_z], normals[mask_z]

        # 随机选取
        idx_x = self.rng.choice(len(px), size=min(nx,len(px)), replace=False)
        idx_y = self.rng.choice(len(py), size=min(ny,len(py)), replace=False)
        idx_z = self.rng.choice(len(pz), size=min(nz,len(pz)), replace=False)

        pts = np.vstack([px[idx_x], py[idx_y], pz[idx_z]])
        nmls = np.vstack([nxm[idx_x], nym[idx_y], nzm[idx_z]])
        return pts, nmls
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        # 均匀采样 X/Y/Z 侧壁
        points, normals = self._sample_uniform_sides(mesh, num_samples)
        ...
        # 遍历采样点
        for i in trange(len(points), desc=f"候选 {os.path.basename(mesh_path)}"):
            ...
            p2 = locs[k_local]
            n2 = self._unit(mesh.face_normals[tri_ids[k_local]])
            ...
            if self.require_through_com:
                ...
                if dist_line > self.through_tol_ratio * max(ex,ey,ez): continue
        ...
        pairs=[]
        for r in results:
            center, R = self._pair_to_pose(r.p1,r.p2,r.n1,r.n2)
            pairs.append({"p1":r.p1,"p2":r.p2,"n1":r.n1,"n2":r.n2,
                          "center":center,"R":R})

        meta={"__meta__":True,"T_mesh_4x4":T_mesh,"scale":scale,"rot_deg":rot,"rot_order":rot_order}
...
# ========== main ==========
def main():
    ap = argparse.ArgumentParser("Batch antipodal grasp generator (uniform XYZ sampling)")
    ...
    ap.add_argument("--rot", type=float, nargs=3, default=(0,0,0))
