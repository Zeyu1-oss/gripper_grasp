...
class AntipodalGraspGenerator:
    ...
    # ---------- 打分 ----------
    def _geom_score_pair(self, p1, p2, n1, n2):
        v = p2 - p1
        d = np.linalg.norm(v)
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        for i in trange(num_samples, desc=f"候选 {os.path.basename(mesh_path)}"):
            ...
            p2 = locs[0]
            n2 = self._unit(mesh.face_normals[tri_ids[0]])
            if np.dot(n1,n2) > -cos_opp: continue
            gscore = self._geom_score_pair(p1,p2,n1,n2)
            candidates.append((p1,p2,n1,n2,gscore))
            if len(candidates) >= 5000: break

        # 打分
        results=[]
        for (p1,p2,n1,n2,gscore) in tqdm(candidates, desc="力闭合打分"):
            eps_fc, wscore = self._wrench_quality(p1,p2,n1,n2,mesh)
            total = 0.1*gscore + 0.9*np.log1p(wscore)
            results.append(PairScored(p1,p2,n1,n2,gscore,eps_fc,wscore,total))
