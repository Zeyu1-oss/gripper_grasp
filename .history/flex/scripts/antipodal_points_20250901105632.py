...
class AntipodalGraspGenerator:
    ...
    def _contact_wrench_rays(self, p, n, com, torque_scale):
        ...
        W /= (np.linalg.norm(W, axis=0, keepdims=True) + 1e-12)  # 列归一化更稳
        return W
    ...
    def _pca_fit_transform(self, X, k=3):
        """
        返回：Z(n,k)、components(Vt)、explained_ratio、mu、std
        """
    ...
    def _diversity_stats(self, Z, max_pairs=1500):
        """
        在 PCA 空间里做多样性度量：
        - pairwise 距离（当 n>max_pairs 时对随机子集计算）
        - 平均最近邻距离（全量）
        """
        n = Z.shape[0]
        if n == 0:
        ...
        # pairwise：对随机子集计算，避免 O(n^2) 太重
        if n > max_pairs:
            idx = self.rng.choice(n, size=max_pairs, replace=False)
            Zp = Z[idx]
        else:
            Zp = Z
        ...
        if m >= 2:
            G = Zp @ Zp.T
            nrm2 = np.sum(Zp*Zp, axis=1, keepdims=True)
            D2 = nrm2 + nrm2.T - 2*G
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        # ε 过滤
        filt = [c for c in cand if c["eps"] >= eps_min]
        if len(filt) == 0:
        ...
        if diverse_select:
            ...
            results = [filt[i] for i in sel_idx]
        else:
            order = np.argsort([-c["eps"] for c in filt])[:min(topk, len(filt))]
            results = [filt[i] for i in order]
