...
if __name__ == "__main__":
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        ...
        for step in range(100):
            data.ctrl[gripper_motor_id] = min(1.0, step * 0.01)  # 从0到1逐步增加
            ...
            print(f"Step {step}: 夹爪开口 = {opening:.4f}m, 控制信号 = {data.ctrl[gripper_motor_id]:.2f}")
    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        for step in range(2000):
            ...
            if step % 100 == 0:
                viewer.sync()
                opening = get_gripper_opening(data, model)
                print(f"模拟步数 {step}: 夹爪开口 = {opening:.4f}m")
