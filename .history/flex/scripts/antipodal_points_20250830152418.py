...
def is_antipodal(p1, n1, p2, n2, mu):
    """检查是否满足摩擦锥下的antipodal条件（与你原逻辑一致）"""
    u = p2 - p1
    d = np.linalg.norm(u)
    if d < 1e-6:
        return False
...
def sample_antipodal_pairs_ray(mesh,
                               ...
                               opp_angle_deg=30.0,      # 反法向粗过滤角度容差
                               cone_half_deg=10.0,      # 围绕 -n1 的圆锥半角
                               rays_per_point=6,        # 每个点发射的射线数
                               dedup=False,             # 是否对抓取中心去重
                               dedup_voxel=0.01,        # 去重体素大小（米）
                               seed=0):
    """
    从表面点出发，多方向射线找对置点，生成antipodal抓取对
    返回：[(p1, p2, n1, n2), ...]，保持你的保存格式不变（np.save 可直接保存）
    """
    ...
    if wmin is None: wmin = 0.2 * min(ex, ey)   # 与你原逻辑一致
    ...
    def _center_cell(c):
        if dedup:
            key = tuple((np.floor(c / dedup_voxel)).astype(int).tolist())
            return key
        else:
            return None
    ...
    for i in trange(num_samples, desc="射线寻找antipodal对"):
        ...
        for j in range(len(locs)):
            ...
            # 可选：连线是否“穿过整体内部”（watertight 时更可靠）
            ...
            # 可选：中点在体内（仅当 watertight）
            if mesh.is_watertight:
                mid = 0.5*(p1 + p2)
                inside = mesh.contains([mid])[0]
                if not inside:
                    continue
...
if __name__ == "__main__":
    ...
    # 直接调用改进版（保存格式不变）
    pairs = sample_antipodal_pairs_ray(
        mesh,
        mu=0.5,
        num_samples=5000,
        wmin=None, wmax=None,
        ...
        cone_half_deg=10.0,
        rays_per_point=8,
        dedup=False,           # 如需减少重复抓取，改为 True
        dedup_voxel=0.01,
        seed=42
    )
    print(f"实际采样到 {len(pairs)} 对antipodal点 (ray-cone method)")

    if len(pairs) > 0:
        np.save(save_path, pairs)
        print(f"✅ 已保存 {len(pairs)} 对antipodal点到 {save_path}")
    else:
        print("❌ 未找到足够的antipodal点对")# import numpy as np
# import trimesh
# import os
# from tqdm import trange
