...
class AntipodalGraspGenerator:
    ...
    # ========== 采样 ==========
    def _uniform_box_samples(self, mesh, num_samples):
        """在 bounding box 六个面均匀采样，并投射到真实表面"""
        bounds = mesh.bounds
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        points, normals = self._uniform_box_samples(mesh, num_samples)

        candidates=[]
        for i in tqdm(range(len(points)), desc="候选生成"):
            p1, n1 = points[i], normals[i]
            axis = -n1
            origins = np.array([p1 - 1e-5*n1])
            dirs = np.array([self._unit(axis)])
            locs, _, tri_ids = trimesh.ray.ray_triangle.RayMeshIntersector(mesh).intersects_location(
            ...
            total = 0.1*gscore + 0.9*np.log1p(eps)
            candidates.append(PairScored(p1,p2,n1,n2,width,gscore,eps,total))
