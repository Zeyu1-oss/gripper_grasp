...
# 几何分（只考虑摩擦裕度+反法向）
def geom_score_pair(p, q, n_p, n_q, mu):
...
def main():
    ...
    for i in trange(len(points), desc="生成候选(射线)"):
        ...
        p2 = locs[0]
        n2 = _unit(mesh.face_normals[tri_ids[0]])
        gscore = geom_score_pair(p1,p2,n1,n2,args.mu)
        if gscore <= 0: continue
        candidates.append((p1,p2,n1,n2,gscore))
        if len(candidates) >= args.max_keep: break
    ...
    for (p1,p2,n1,n2,gscore) in tqdm(candidates, desc="力闭合打分"):
        ...
        results.append(PairScored(p1,p2,n1,n2,gscore,eps,wscore,total))
    results.sort(key=lambda r:-r.total)
    if args.topk>0: results = results[:args.topk]

    out_dir = os.path.abspath(args.out_dir)
    os.makedirs(out_dir,exist_ok=True)
    out_path = os.path.join(out_dir,f"{args.out_name}_no_width.npy")
