...
class AntipodalGraspGenerator:
    ...
    # ---------- 多样性：特征、PCA、FPS ----------
    def _make_feature(self, rec):
        p1 = rec["p1"]; p2 = rec["p2"]; n1 = rec["n1"]; n2 = rec["n2"]
        center = 0.5*(p1+p2)
        u = self._unit(p2 - p1)
        width = np.linalg.norm(p2 - p1)
        return np.hstack([center, u, [width], n1, n2])  # 3+3+1+3+3 = 13D

    def _pca_project(self, X, k=3):
        Xc = X - X.mean(axis=0, keepdims=True)
        U, S, Vt = np.linalg.svd(Xc, full_matrices=False)
        comps = Vt[:k].T            # d x k
        Y = Xc @ comps              # N x k
        expvar = (S[:k]**2) / (S**2).sum()
        return Y, expvar

    def _fps(self, Y, K, start_idx=None):
        N = Y.shape[0]
        if N <= K: return np.arange(N, dtype=int)
        if start_idx is None:
            start_idx = int(np.argmax(np.linalg.norm(Y, axis=1)))
        sel = [start_idx]
        dist = np.full(N, np.inf)
        diff = Y - Y[start_idx]
        dist = np.minimum(dist, np.sum(diff*diff, axis=1))
        for _ in range(1, K):
    ...
    def _diversity_stats(self, Y, idx, tag="ALL"):
        if len(idx) < 2:
            print(f"[Diversity@{tag}] 样本太少")
            return
        P = Y[idx]
        ...
        pair = np.sqrt(D2[iu])
        print(f"[Diversity@{tag}] pair_mean={pair.mean():.4f}, pair_min={pair.min():.4f}, "
              f"pair_max={pair.max():.4f}, nn_mean={nn.mean():.4f}")

    # ---------- 主流程 ----------
    def generate_for_mesh(self, mesh_path, scale=0.01,
                          num_samples=20000, topk=1000,
                          out_path=None, eps_thresh=1e-12,
                          use_diversity=True):
        ...
        if len(candidates) == 0:
            print("[EPS 统计] 没有通过筛选的抓取。")
            results = []
        else:
            # 先按 eps 排序（大到小）
            candidates.sort(key=lambda r: -r["eps"])

            # 多样性选择（PCA + FPS）
            if use_diversity and len(candidates) > topk:
                X = np.stack([self._make_feature(c) for c in candidates], axis=0)
                Y, exp = self._pca_project(X, k=3)
                print(f"[PCA] explained variance ratio (前 3 PCs): {exp[0]:.3f}, {exp[1]:.3f}, {exp[2]:.3f}")
                self._diversity_stats(Y, np.arange(len(candidates)), tag="ALL")

                # 用 eps 最大的样本作为 FPS 起点
                start_idx = 0
                order = self._fps(Y, topk, start_idx=start_idx)
                results = [candidates[i] for i in order]

                Y_fin = Y[order]
                self._diversity_stats(Y_fin, np.arange(len(results)), tag="FINAL")
            else:
                results = candidates[:topk]

            # ε 统计
            eps_vals = np.array([c["eps"] for c in results], float)
            print(f"[EPS 统计] mean={eps_vals.mean():.4e}, "
                  f"max={eps_vals.max():.4e}, min={eps_vals.min():.4e}, n={len(results)}")

        # 保存（字典列表 + meta，兼容你的 visualize 脚本）
        if out_path:
            os.makedirs(os.path.dirname(out_path), exist_ok=True)
            meta = {
                "__meta__": True,
                "scale": scale,
                "note": "full-surface-sampling; farthest-hit; eps>0; PCA+FPS diversity",
                "params": {
                    "mu": self.mu,
                    "opp_angle_deg": self.opp_angle_deg,
                    "m_dirs": self.m_dirs,
                    "wmin": 0.01,
                    "wmax": 0.05,
                    "require_through_com": self.require_through_com,
                    "through_tol_ratio": self.through_tol_ratio,
                    "eps_thresh": eps_thresh
                }
            }
            payload = np.array(results + [meta], dtype=object)
...
# ========== CLI ==========
def main():
    ap = argparse.ArgumentParser("Antipodal grasp generator (全表面 + 最远穿透 + ε + 多样性)")
    ...
    ap.add_argument("--no_diversity", action="store_true", help="关闭 PCA+FPS 多样性挑选")
    args = ap.parse_args()
