...
def sample_antipodal_pairs(mesh, mu=0.5, approach_dir=np.array([1,0,0]), angle_thresh=np.pi/8, num_pairs=10000, num_surface_samples=20000):
    ...
    for _ in trange(num_pairs * 2):  # 多采样一些，防止采样效率低
        i = rng.choice(idx1)
        j = rng.choice(idx2)
...
if __name__ == "__main__":
    ...
    print(f"实际采样到 {len(pairs)} 对antipodal点")
    ...
    if not (np.all(np.isfinite(p1)) and np.all(np.isfinite(p2)) and np.all(np.isfinite(n1)) and np.all(np.isfinite(n2))):
        print(f"第{idx}对有非法数值")