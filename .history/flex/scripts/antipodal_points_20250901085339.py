...
class AntipodalGraspGenerator:
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01,
        ...
        for i in tqdm(range(len(points)), desc=f"候选生成 {os.path.basename(mesh_path)}"):
            ...
            eps_values.append(eps)   # 收集 eps

            total = 0.3*gscore + 0.7*np.log1p(eps)
        ...
        # 打印 eps 统计信息
        if len(eps_values) > 0:
            print(f"[EPS 统计] 平均 = {np.mean(eps_values):.4e}, "
                  f"最大 = {np.max(eps_values):.4e}, "
                  f"最小 = {np.min(eps_values):.4e}")
        else:
            print("[EPS 统计] 没有有效的 eps")
