...
class AntipodalGraspGenerator:
    ...
    def _geom_score_pair(self, p1, p2, n1, n2, wmin, wmax):
        ...
        # 摩擦锥条件
        friction_angle = np.arctan(self.mu)
        cos_th = np.cos(friction_angle)
        
        # 计算力闭合裕度
        margin_p = max(0, -cos_th - np.dot(n1, u))
        margin_q = max(0, np.dot(n2, u) - cos_th)
        
        # 对握性评分
        antipodality = -np.dot(n1, n2)
        
        # 间距惩罚
        if d < wmin:
            width_penalty = (wmin - d) ** 2
        elif d > wmax:
            width_penalty = (d - wmax) ** 2
        else:
            mid = 0.5 * (wmin + wmax)
            width_penalty = 0.25 * ((d - mid) / (0.5 * (wmax - wmin) + 1e-9)) ** 2
        
        # 综合评分
        score = (-margin_p - margin_q) + 0.5 * antipodality - 0.1 * width_penalty
        return score
    ...
    def generate_for_mesh(self, mesh_path, scale=0.01, num_samples=20000, 
                         topk=1000, out_path=None, visualize=False):
        """生成对握抓取候选"""
        # 加载和预处理网格
        ...
        com = mesh.center_mass if hasattr(mesh, 'is_watertight') and mesh.is_watertight else mesh.centroid
        ...
        # 进度条
        pbar = tqdm(total=len(points), desc=f"生成抓取候选 {os.path.basename(mesh_path)}")
        ...
        for i in range(len(points)):
            ...
            # 求交
            locations, index_ray, index_tri = ray_engine.intersects_id(
                [origins], [ray_dir], multiple_hits=True, return_locations=True
            )
            ...
            # 找到最远的交点
            distances = np.linalg.norm(locations - p1, axis=1)
            farthest_idx = np.argmax(distances)
            p2 = locations[farthest_idx]
            n2 = mesh.face_normals[index_tri[farthest_idx]]
            
            # 检查对握条件
            if np.dot(n1, n2) > -np.cos(np.deg2rad(self.opp_angle_deg)):
                pbar.update(1)
                continue
            ...
            if self.require_through_com:
                line_dir = (p2 - p1) / (width + 1e-12)
                com_dist = np.linalg.norm(np.cross(com - p1, line_dir))
                if com_dist > self.through_tol_ratio * bbox_diag:
                    pbar.update(1)
                    continue
            ...
            # 计算力闭合度量
            W_matrix = np.column_stack([
                np.hstack([n1, np.zeros(3)]),
                np.hstack([n2, np.zeros(3)])
            ])
            eps = self._epsilon_qp(W_matrix)
            eps_values.append(eps)
            
            # 综合评分
            total_score = 0.3 * gscore + 0.7 * np.log1p(eps)
        ...
        # 筛选和排序
        eps_thresh = 1e-10
        valid_candidates = [c for c in candidates if c["eps"] > eps_thresh]
        valid_candidates.sort(key=lambda x: x["total"], reverse=True)
        results = valid_candidates[:topk]
        
        # 输出统计信息
        if eps_values:
            eps_arr = np.array(eps_values)
            print(f"[统计] 有效候选: {len(valid_candidates)}/{len(candidates)}")
            print(f"[EPS] 均值: {eps_arr.mean():.3e}, 范围: [{eps_arr.min():.3e}, {eps_arr.max():.3e}]")
        
        # 保存结果
        if out_path and results:
            os.makedirs(os.path.dirname(out_path), exist_ok=True)
            np.save(out_path, results, allow_pickle=True)
            print(f"✅ 保存 {len(results)} 个抓取到 {out_path}")
