    ...
    with mujoco.viewer.launch_passive(model, data) as viewer:
        while viewer.is_running():
            ...
            if now - last_print >= PRINT_PERIOD:
                print(f"tip_z={tip_z:.3f}  z_goal_tip={z_goal_tip:.3f}  desire_yaw={desired_yaw:.3f}  "
                      f"yaw_cur={yaw_cur:+.3f} yaw_cmd={target_rot:+.3f}  "
            ...
            if phase == "align_xy":
                ...
                # 位置式控制：直接设置目标角度
                target_rot = err  #  desired_yaw是lego的旋转,
                data.ctrl[aid_rot] = float(np.clip(target_rot, loR, hiR))
